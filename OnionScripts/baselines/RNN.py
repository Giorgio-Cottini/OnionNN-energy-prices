...
class DeepLSTM(nn.Module):
    def __init__(self,
        ...
        #————— Fusion + Output head 
        # Here i set output dimension (24h)
        self.dense_out = nn.Sequential(
            nn.Linear(hidden_size + 1, 128, device = device),
            nn.GELU(),
    ...
    def set_trainable(
        ...
        *,                          # keyword-only
        ...
        bzn_embed  : bool = True,
        lstm       : bool = True,
        dense      : bool = True       # head(s) default to trainable
    ) -> None:
    ...
    def forward(
        ...
        # Final input: numeric features + embeddings
        ...
        #──── Concatenate gas price 
        # -> detach so that information about it's evolution is not encoded in the LSTM, parameters before and 
        #    after its insertion have different gradients
        combined_features = torch.cat([lstm_last_step, gas_price.detach()], dim = -1)  # (B, H + 1)

        #──── Output layer
        out = self.dense_out(combined_features)  # (B, output_size)
